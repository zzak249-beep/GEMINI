...
async def _health(request):
    ...
    return web.Response(status=status,
                        text=f"OK cycles={_cycle_count} last={age:.0f}s")
        ...
        while not _shutdown:
            try:
                ...
                if now.day != last_scan_day:
                    try:
                        ...
                        await tg.daily_summary(session, trader.daily_trades,
                                               trader.daily_wins, trader.daily_pnl,
                                               balance)
                        trader.reset_daily()
                        active_pairs  = await scan_explosive_pairs(client, session, balance)
                        last_scan_day = now.day
                    except Exception as e:
                        log.error(f"Reset diario: {e}")
                ...
                try:
                    ...
                    log.warning(f"refresh_live: {e}")

                active_count = sum(1 for p in trader.positions.values() if not p.closed)
                log.info(
                    f"━━ #{_cycle_count} bal={balance:.2f} "
                    f"pos={active_count}/{config.MAX_POSITIONS} "
                    f"pnl={trader.daily_pnl:+.4f} pairs={len(active_pairs)}"
                )
                ...
                if active_pairs and not trader.paused:
                    ...
                    for sym, res in zip(active_pairs, results):
                        if isinstance(res, Exception):
                            log.error(f"[{sym}] {type(res).__name__}: {res}")
                elif trader.paused:
                    log.warning("⏸️  Pausado por pérdida diaria")
                ...
                consec_errors += 1
                log.exception(f"Loop error #{consec_errors}: {e}")
                try:
                    await tg.error_alert(session, f"Loop error: {e}")
                except Exception:
                    pass
