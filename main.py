...
_shutdown=False; _last_cycle=datetime.now(timezone.utc); _cycle=0

async def _health(r):
    age=(datetime.now(timezone.utc)-_last_cycle).total_seconds()
    return web.Response(status=200 if age<300 else 503,
                        text=f"OK cycles={_cycle} age={age:.0f}s")

async def main():
    global _last_cycle, _cycle, _shutdown
    app=web.Application()
    app.router.add_get("/",_health); app.router.add_get("/health",_health)
    runner=web.AppRunner(app); await runner.setup()
    await web.TCPSite(runner,"0.0.0.0",config.PORT).start()
    log.info(f"🌐 Health :{config.PORT}")
    ...
    async with aiohttp.ClientSession(connector=connector,timeout=timeout) as session:
        ...
        last_day=last_scan.day
        log.info(f"🚀 {len(pairs)} pares: {pairs}")
        errs=0

        while not _shutdown:
            try:
                now=datetime.now(timezone.utc); _last_cycle=now; _cycle+=1

                # Rescan cada SCAN_INTERVAL_H horas
                if (now-last_scan).total_seconds() >= config.SCAN_INTERVAL_H*3600:
                    try:
                        ...
                        last_scan=now
                        log.info(f"🔄 Re-scan: {len(pairs)} pares")
                ...
                errs+=1; log.exception(f"loop error #{errs}: {e}")
