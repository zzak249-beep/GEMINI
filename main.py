"""
QF×JP Bot v6.8 — Main
FIX v6.8: versión sincronizada con position_manager v6.8 y risk_manager v6.8
FastAPI: /health + /status + /close/{symbol} + /positions
...
async def lifespan(app: FastAPI):
    ...
    log.info("═" * 55)
    log.info("  QF×JP Bot v6.8 — PREDATOR·ENTRY")
    ...
    log.info("  FIX v6.8: PnL sin LEVERAGE, BE=True en reconcile, reversal post-TP1")
    log.info("═" * 55)
    ...
    log.info("Loops iniciados (scanner + monitor)")
...
app = FastAPI(title="QF×JP Bot v6.8", docs_url=None, redoc_url=None, lifespan=lifespan)


@app.get("/health")
async def health():
    return {"status": "ok", "version": "6.8", "mode": C.MODE}
