...
async def bot_start(session):
    await send(session,
        ...
        f"🔴 SHORT:        <code>+{config.SHORT_PIPS}p encima GREEN + EMA cross + ADX<{config.ADX_MAX}</code>\n"
        f"🟢 LONG:         <code>-{config.LONG_PIPS}p debajo RED + EMA cross + ADX<{config.ADX_MAX}</code>\n"
