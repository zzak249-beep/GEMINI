"""
QF×JP Bot v6.6 — Indicators PINE SYNC (FIX SQP look-back)
═══════════════════════════════════════════════════════════════════════════════
NUEVO en v6.6:
  ✅ FIX SQP (Squeeze Pull-Back): sqp_long/sqp_short ahora buscan el
     squeeze fire en las últimas 5 barras, no solo en la barra actual.
     El fire y el pull-back son típicamente velas DISTINTAS — el diseño
     anterior requería que ambos coincidieran simultáneamente, así que
     este setup casi nunca se detectaba en producción aunque hubiera
     squeeze fires reales frecuentes. Se llama detect_squeeze() hasta 5
     veces extra por símbolo (cada una O(n) sobre los últimas 25 barras)
     — coste despreciable vs el resto del análisis.

Sin más cambios vs v6.5 (Pine Sync v3.6).
═══════════════════════════════════════════════════════════════════════════════
"""
import logging
import math
...
def calc_mfi_divergence(close, mfi_arr: np.ndarray, window_long: int = 14,
    ...
    if len(c) < window_long + 2:
        return False, False
    mfi = mfi_arr
    bull_div_n = (c[-1] < c[-window_long - 1] and mfi[-1] > mfi[-window_long - 1] and mfi[-1] < 50)
    bear_div_n = (c[-1] > c[-window_long - 1] and mfi[-1] < mfi[-window_long - 1] and mfi[-1] > 50)
    bull_div_s = bear_div_s = False
    if len(c) > window_short + 1:
        bull_div_s = (c[-1] < c[-window_short - 1] and mfi[-1] > mfi[-window_short - 1] and mfi[-1] < 50)
        bear_div_s = (c[-1] > c[-window_short - 1] and mfi[-1] < mfi[-window_short - 1] and mfi[-1] > 50)
...
def calc_vdi_accel(vdi_z: np.ndarray, threshold: float = 0.5) -> tuple[bool, bool]:
    ...
    return (curr > prev2) and (curr > threshold), (curr < prev2) and (curr < -threshold)
...
def detect_squeeze(close, high, low, length: int = 20,
    ...
    sq_fire    = not sq_on_curr and sq_on_prev
...
def detect_eqh_eql(high, low, close, atr: float,
    ...
    tol    = atr * tol_atr
    curr_h, curr_l, curr_c = h[-1], l[-1], c[-1]
    eqh_detected = eql_detected = False
    eqh_price = eql_price = None
    for i in range(2, min(lookback + 1, len(h))):
        if not eqh_detected and abs(curr_h - h[-i]) < tol and h[-i] > 0:
            eqh_detected, eqh_price = True, h[-i]
        if not eql_detected and abs(curr_l - l[-i]) < tol and l[-i] > 0:
            eql_detected, eql_price = True, l[-i]
    eqh_sweep = eqh_detected and eqh_price and h[-1] > eqh_price and curr_c < eqh_price
    eql_sweep = eql_detected and eql_price and l[-1] < eql_price and curr_c > eql_price
    return eqh_detected, eql_detected, bool(eqh_sweep), bool(eql_sweep)
...
def detect_ob_and_premium(high, low, close, open_, atr: float,
    ...
    if len(c) < ob_bars + 3:
        return {k: False for k in ["in_bull_ob","in_bear_ob","in_bull_ob_premium",
                                    "in_bear_ob_premium","ob_bull_approach","ob_bear_approach"]}
    curr = c[-1]
    bob_hi = bob_lo = sob_hi = sob_lo = None
    for i in range(1, min(ob_bars, len(c) - 2)):
        if (c[-i] - o[-i]) > atr * ob_imp and c[-(i+1)] < o[-(i+1)]:
            bob_hi, bob_lo = o[-(i+1)], c[-(i+1)]
            break
    for i in range(1, min(ob_bars, len(c) - 2)):
        if (o[-i] - c[-i]) > atr * ob_imp and c[-(i+1)] > o[-(i+1)]:
            sob_hi, sob_lo = c[-(i+1)], o[-(i+1)]
            break
    result = {
        "in_bull_ob": False, "in_bear_ob": False,
        "in_bull_ob_premium": False, "in_bear_ob_premium": False,
        "ob_bull_approach": False, "ob_bear_approach": False,
        "bull_ob_hi": bob_hi, "bull_ob_lo": bob_lo,
        "bear_ob_hi": sob_hi, "bear_ob_lo": sob_lo,
    }
    if bob_hi is not None and bob_lo is not None:
        ...
        result["ob_bull_approach"]   = not result["in_bull_ob"] and prem_lo - atr * obp2_dist < curr < prem_lo
    if sob_hi is not None and sob_lo is not None:
        ...
        result["ob_bear_approach"]   = not result["in_bear_ob"] and prem_hi < curr < prem_hi + atr * obp2_dist
    return result
...
def detect_dark_pool(high, low, close, open_, volume,
    ...
    return vol_spike and rng_narrow and c[-1] > o[-1], vol_spike and rng_narrow and c[-1] < o[-1]
...
def composite_score(
    direction: str, adx: float, cvd: float, momentum: float,
    mfi: float, vdi: float, structure: str, tl_break: str,
    htf_s: float, fvg: str, funding: float = 0.0,
    rsi3_bull: bool = False, rsi3_bear: bool = False,
    sqp_setup: bool = False, vdi_accel: bool = False,
    near_vwap_lo: bool = False, near_vwap_hi: bool = False,
    ob_premium: bool = False, eql_sweep: bool = False,
    eqh_sweep: bool = False, breaker_bl: bool = False,
    mfi_div: bool = False,
) -> float:
    s = 0.0
...
def calc_conviction(
    direction: str, norm_score: float, sig_alive: bool,
    htf_score_val: int, asym_bull: bool, asym_bear: bool,
    sell_exhausted: bool, buy_exhausted: bool, tl_break: str,
    dp_buy: bool, dp_sell: bool, cvd_rising: bool,
    sq_bull: bool, sq_bear: bool, in_bull_fvg: bool, in_bear_fvg: bool,
    in_bull_ob: bool, in_bear_ob: bool, in_brk_bull: bool, in_brk_bear: bool,
    vdi_bull: bool, vdi_bear: bool, in_bull_ob_premium: bool, in_bear_ob_premium: bool,
    mfi_os: bool, mfi_ob: bool, mfi_bull_div: bool, mfi_bear_div: bool,
    eql_sweep: bool, eqh_sweep: bool, rsi3_bull: bool, rsi3_bear: bool,
    sqp_long: bool, sqp_short: bool, vdi_accel_bull: bool, vdi_accel_bear: bool,
    ...
    if direction == "LONG":
        score  = int(norm_score > 0.10) + int(sig_alive) + int(htf_score_val >= 2)
        score += int(asym_bull) + int(sell_exhausted)
        score += int(tl_break == "LONG" or liq_bull_sweep or choch_bull)
        score += int(dp_buy) + int(cvd_rising)
        score += int(sq_bull or in_bull_fvg or in_bull_ob or in_brk_bull)
        score += int(vdi_bull) + int(in_bull_ob_premium) + int(mfi_os or mfi_bull_div)
        score += int(eql_sweep) + int(rsi3_bull) + int(sqp_long)
        score += int(vdi_accel_bull) + int(near_vwap_lo)
    else:
        score  = int(norm_score < -0.10) + int(sig_alive) + int(htf_score_val >= 2)
        score += int(asym_bear) + int(buy_exhausted)
        score += int(tl_break == "SHORT" or liq_bear_sweep or choch_bear)
        score += int(dp_sell) + int(not cvd_rising)
        score += int(sq_bear or in_bear_fvg or in_bear_ob or in_brk_bear)
        score += int(vdi_bear) + int(in_bear_ob_premium) + int(mfi_ob or mfi_bear_div)
        score += int(eqh_sweep) + int(rsi3_bear) + int(sqp_short)
        score += int(vdi_accel_bear) + int(near_vwap_hi)
    return min(score, 20)

# ── analyze() principal ───────────────────────────────────────────────────────

def analyze(
    symbol: str, klines_3m: list, klines_15m: list,
    klines_1h: list, klines_4h: list, funding_rate: float = 0.0,
) -> Signal:
    ...
    eqh_det, eql_det, eqh_sweep, eql_sweep = detect_eqh_eql(h, l, c, atr, C.EQL_LEN, C.EQL_TOL)

    ob_data = detect_ob_and_premium(h, l, c, o, atr,
                                    ob_imp=1.5, ob_bars=50, prem_pct=0.33,
                                    obp2_dist=C.OBP2_DIST)
    ...
    # ── FIX v6.6: SQP look-back de 5 barras ──────────────────────────────────
    # Antes: sqp_long/short se activaba SOLO si el fire ocurría en la barra
    # actual — fire y pull-back simultáneos en la misma vela. Esto es casi
    # imposible: el fire es el momento en que el squeeze libera, y el
    # pull-back es la vela siguiente (precio vuelve a la EMA y rebota).
    # Ahora: buscamos el fire en las últimas 5 barras. detect_squeeze(c[:-b])
    # devuelve True si la vela c[-(b+1)] liberó el squeeze, así que b=1 →
    # el fire fue hace 1 barra, y la barra actual es el pull-back potencial.
    _sqp_long = _sqp_short = False
    _SQP_BARS = 5
    for _b in range(1, _SQP_BARS + 2):
        if len(c) <= _b + 22:
            break
        _sf_b, _sf_s, _ = detect_squeeze(c[:-_b], h[:-_b], l[:-_b], 20)
        if _sf_b and not _sqp_long:
            _sqp_long = (cvd_rising and
                         c[-1] <= ema20[-1] * 1.001 and
                         c[-1] > c[-2])
        if _sf_s and not _sqp_short:
            _sqp_short = (not cvd_rising and
                          c[-1] >= ema20[-1] * 0.999 and
                          c[-1] < c[-2])
        if _sqp_long and _sqp_short:
            break
    sqp_long, sqp_short = _sqp_long, _sqp_short
    ...
    conviction = calc_conviction(
        direction=direction, norm_score=cvd_val, sig_alive=sig_alive,
        htf_score_val=htf_aligned, asym_bull=asym_bull, asym_bear=asym_bear,
        sell_exhausted=sell_exhausted, buy_exhausted=buy_exhausted,
        tl_break=tl_break, dp_buy=dp_buy, dp_sell=dp_sell,
        cvd_rising=cvd_rising, sq_bull=sq_fire_bull, sq_bear=sq_fire_bear,
        in_bull_fvg=(fvg == "BULL"), in_bear_fvg=(fvg == "BEAR"),
        in_bull_ob=in_bull_ob, in_bear_ob=in_bear_ob,
        in_brk_bull=in_brk_bull, in_brk_bear=in_brk_bear,
        vdi_bull=vdi_bull, vdi_bear=vdi_bear,
        in_bull_ob_premium=in_bull_ob_premium, in_bear_ob_premium=in_bear_ob_premium,
        mfi_os=mfi_os, mfi_ob=mfi_ob, mfi_bull_div=mfi_bull_div, mfi_bear_div=mfi_bear_div,
        eql_sweep=eql_sweep, eqh_sweep=eqh_sweep, rsi3_bull=rsi3_bull, rsi3_bear=rsi3_bear,
        sqp_long=sqp_long, sqp_short=sqp_short,
        vdi_accel_bull=vdi_accel_bull, vdi_accel_bear=vdi_accel_bear,
        near_vwap_lo=near_vwap_lo, near_vwap_hi=near_vwap_hi,
        choch_bull=structure in ("CHoCH↑",), choch_bear=structure in ("CHoCH↓",),
    )
    ...
    return Signal(
        ...
        funding_rate=funding_rate, reason="ok",
