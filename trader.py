"""
Trader — ZigZag V32 FINAL
Si ADX < ADX_MIN (señal weak), reduce el size al 50%
para proteger capital en condiciones de mercado lateral.
"""
import asyncio
import logging
import math
...
class Trader:
    ...
    async def process_pair(self, symbol: str, balance: float):
        ...
        try:
            ...
            log.warning(f"  [{symbol}] get_klines: {e}")
            return
    ...
    async def _enter_trade(self, symbol: str, sig: dict, balance: float):
        try:
            ...
            atr      = sig["atr"]
            rr       = sig.get("rr", 0.0)
            adx_ok   = sig.get("adx_ok", True)

            if abs(entry - sl) == 0 or entry == 0:
                return

            # Size normal; si ADX débil → 50% del size
            risk_pct  = config.RISK_PCT if adx_ok else config.RISK_PCT * 0.5
            risk_usdt = balance * (risk_pct / 100)
            qty_raw   = (risk_usdt * config.LEVERAGE) / entry
            qty       = math.floor(qty_raw * 10000) / 10000

            if qty <= 0:
                log.warning(f"[{symbol}] qty={qty:.8f} ≤ 0 | balance={balance:.2f} risk={risk_usdt:.4f}")
                return

            size_note = "" if adx_ok else " [50% size — ADX débil]"
            log.info(f"[{symbol}] 🚀 {side} qty={qty:.4f} entry={entry:.6g} SL={sl:.6g} TP={tp:.6g}{size_note}")
            ...
            if code != 0:
                ...
                log.error(f"[{symbol}] ❌ Rechazada code={code}: {err}")
        ...
        try:
            ...
            if pos.elapsed_min >= config.TIME_STOP_MINUTES:
                ...
                await tg.trade_exit(self.session, symbol, pos.side, pos.entry,
                                    exit_price, pnl, pnl_pct, "⏱ TIME-STOP")
                return
            ...
            if symbol not in self._live_pos_cache:
                ...
                log.info(f"[{symbol}] {reason} PnL={pnl:+.4f} USDT held={pos.elapsed_min:.1f}min")
