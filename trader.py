...
class Position:
    def __init__(self,symbol,side,entry,sl,tp,qty,green,red,rr=0.0,atr=0.0):
        self.symbol=symbol;self.side=side;self.entry=entry;self.sl=sl;self.tp=tp
        self.qty=qty;self.green=green;self.red=red;self.rr=rr;self.atr=atr
        self.open_time=time.time();self.closed=False
...
class Trader:
    def __init__(self,client:BingXClient,session:aiohttp.ClientSession):
        self.client=client;self.session=session;self.strategy=ChannelFadeSignal()
        self.positions:Dict[str,Position]={};self.daily_pnl=0.0
        self.daily_trades=0;self.daily_wins=0;self.paused=False
    ...
    async def process_pair(self,symbol:str,balance:float):
        ...
        if balance>0 and (self.daily_pnl/balance)*100<=-config.MAX_DAILY_LOSS:
            self.paused=True
            await tg.daily_loss_limit(self.session,self.daily_pnl,config.MAX_DAILY_LOSS,balance); return
        pos=self.positions.get(symbol)
        if pos and not pos.closed:
            await self._monitor_position(symbol); return
        if sum(1 for p in self.positions.values() if not p.closed)>=config.MAX_POSITIONS: return
        ...
        await tg.signal_channel_fade(self.session,symbol,sig["side"],
            ...
            sig["canal_width"],sig["vol_ratio"],sig["adx"],sig["rr"],sig.get("rsi",50.0))
    ...
    async def _enter_trade(self,symbol:str,sig:dict,balance:float):
        try:
            ...
            atr=sig["atr"];rr=sig.get("rr",0.0)
            if abs(entry-sl)==0 or entry==0: return
            if balance<=0:
                log.warning(f"[{symbol}] balance={balance:.2f} — verifica wallet futuros en BingX"); return
            ...
            if qty*entry<config.MIN_NOTIONAL:
                log.warning(f"[{symbol}] notional={qty*entry:.2f}<{config.MIN_NOTIONAL}"); return
            if qty<=0:
                log.warning(f"[{symbol}] qty=0"); return
            ...
            if resp.get("code",-1)!=0:
                await tg.error_alert(self.session,f"[{symbol}] {resp.get('msg',str(resp))}"); return
            pos=Position(symbol,side,entry,sl,tp,qty,sig["green"],sig["red"],rr,atr)
            self.positions[symbol]=pos;self._live_pos_cache.add(symbol);self.daily_trades+=1
            await tg.trade_entry(self.session,symbol,side,entry,sl,tp,qty,
                                 balance,rr,atr,sig["adx"],sig["vol_ratio"],sig.get("rsi",50.0))
            log.info(f"✅ [{symbol}] {side} @ {entry:.5g} RR=1:{rr:.2f}")
        except Exception as e:
            log.exception(f"[{symbol}] enter: {e}")
    ...
    async def _monitor_position(self,symbol:str):
        try:
            ...
            if len(C)<2: return
            current=float(C[-2])
            if (time.time()-pos.open_time)/60>=config.TIME_STOP_MINUTES:
                await self.client.close_position_market(symbol,pos.side,pos.qty)
                pos.closed=True
                await self._record_exit(pos,symbol,"⏱ TIME-STOP",current); return
            if await self._check_trail(pos,symbol,current): return
            if symbol not in self._live_pos_cache:
                pos.closed=True
                reason="TAKE PROFIT ✅" if abs(current-pos.tp)<abs(current-pos.sl) else "STOP LOSS ❌"
                if "TAKE" in reason: self.daily_wins+=1
                await self._record_exit(pos,symbol,reason,current)
        except Exception as e: log.error(f"[{symbol}] monitor: {e}")

    async def _check_trail(self,pos:Position,symbol:str,current:float)->bool:
        if pos.atr==0: return False
        be=config.BREAKEVEN_ATR*pos.atr;tri=config.TRAIL_ATR*pos.atr;trd=config.TRAIL_DIST*pos.atr
        if pos.side=="BUY":
            ...
            if fav>=be and not pos.breakeven_active:
                ...
                log.info(f"[{symbol}] 🔒 BE")
                await tg.send(self.session,f"🔒 <b>BREAKEVEN</b> — <code>{symbol}</code> SL→<code>{pos.entry:.5g}</code>")
            if fav>=tri:
            ...
            if pos.breakeven_active and current<=pos.trail_sl:
                ...
                await self._record_exit(pos,symbol,r,current); return True
        else:
    ...
    async def _record_exit(self,pos:Position,symbol:str,reason:str,exit_price:float=0.0):
        ...
        self.daily_pnl+=pnl
        log.info(f"[{symbol}] PnL={pnl:+.4f} | {reason}")
        await tg.trade_exit(self.session,symbol,pos.side,pos.entry,exit_price,pnl,pnl_pct,reason)
