...
class Position:
    def __init__(self, symbol: str, side: str, entry: float,
        ...
        self.tp     = tp
        self.qty    = qty
        self.green  = green   # nivel línea verde en el momento de entrada
        self.red    = red     # nivel línea roja  en el momento de entrada
        self.closed = False
    ...
    async def _enter_trade(self, symbol: str, sig: dict, balance: float):
        try:
            ...
            self.positions[symbol] = Position(
                symbol, side, entry, sl, tp, qty, sig["green"], sig["red"]
            )
            ...
            log.info(f"✅ [{symbol}] {side} @ {entry:.4f} | SL={sl:.4f} TP={tp:.4f} qty={qty:.4f} RR=1:{rr:.1f}")
    ...
    async def _monitor_position(self, symbol: str):
        try:
            ...
            if symbol not in self._live_pos_cache:
                ...
                # Precio de salida: última vela cerrada
