...
def stdev(arr: np.ndarray, p: int) -> np.ndarray:
    out = np.full(len(arr), np.nan)
    for i in range(p-1, len(arr)): out[i] = arr[i-p+1:i+1].std(ddof=0)
    return out

def atr_series(h, l, c, p: int) -> np.ndarray:
    tr = np.empty(len(c)); tr[0] = h[0]-l[0]
    for i in range(1, len(c)):
        tr[i] = max(h[i]-l[i], abs(h[i]-c[i-1]), abs(l[i]-c[i-1]))
    return ema(tr, p)

def adx_series(h, l, c, p: int):
    n = len(c)
...
def analizar_par(klines_3m: list, klines_15m: list) -> Optional[dict]:
    ...
    phv = [(i,v) for i,v in enumerate(ph_arr) if not np.isnan(v)]
    plv = [(i,v) for i,v in enumerate(pl_arr) if not np.isnan(v)]
    tl_break_long = tl_break_short = False
    if len(phv)>=2:
        (pb2,ph2),(pb1,ph1) = phv[-2],phv[-1]
        if ph2>ph1 and (n-1-pb2)<=I_TLB:
            sl=(ph1-ph2)/max(pb1-pb2,1)
            if c[-1] > ph1+sl*(n-1-pb1)+atr_now*I_TLM: tl_break_long=True
    if len(plv)>=2:
        (lb2,pl2),(lb1,pl1) = plv[-2],plv[-1]
        if pl2<pl1 and (n-1-lb2)<=I_TLB:
            sl=(pl1-pl2)/max(lb1-lb2,1)
            if c[-1] < pl1+sl*(n-1-lb1)-atr_now*I_TLM: tl_break_short=True

    # L8 Swing exhaustion
    win = min(I_HLW, n)
    plr = [(i,v) for i,v in enumerate(pl_arr[-win:]) if not np.isnan(v)]
    phr = [(i,v) for i,v in enumerate(ph_arr[-win:]) if not np.isnan(v)]
    hl_c = sum(1 for j in range(1,len(plr)) if plr[j][1]>plr[j-1][1])
    lh_c = sum(1 for j in range(1,len(phr)) if phr[j][1]<phr[j-1][1])
    sell_exhausted = hl_c >= I_HLC
    buy_exhausted  = lh_c >= I_HHC
...
def build_alerta(par: dict) -> str:
    ...
    lines=[
        ...
        f"  SWING  `{'HL↑' if par['sell_exhausted'] else 'LH↓' if par['buy_exhausted'] else '—'}`",
        f"  DP     `{'↑' if par['dp_buy'] else '↓' if par['dp_sell'] else '—'}`",
        f"  FVG    `{'↑' if par['in_bull_fvg'] else '↓' if par['in_bear_fvg'] else '—'}` | OB `{'↑' if par['in_bull_ob'] else '↓' if par['in_bear_ob'] else '—'}`",
        f"  CVD    `{'DIV↑' if par['cvd_bull_div'] else 'DIV↓' if par['cvd_bear_div'] else '↑' if par['cvd_rising'] else '↓'}`",
        f"  SQ     `{'🔥↑' if par['sq_bull'] else '🔥↓' if par['sq_bear'] else 'COMP' if par['sq_on'] else '—'}`",
        f"  EXEC   `{'OK' if par['exec_ok'] else 'BLOQ'}`",
        f"{'─'*30}",
        f"📌 {'SL LONG' if is_long else 'SR SHORT'}: `{par['last_sl'] if is_long else par['last_sh']}`",
    ]
    if AUTO_TRADE and (is_sup or is_fuel):
        lines.append(f"🤖 *AUTO-TRADE {'ABIERTO' if par['symbol'] in trades_abiertos else 'PENDIENTE'}*")
    else:
        lines.append(f"👆 *Verifica en TradingView 3m + QF×JP*")
    return "\n".join(lines)
...
def build_resumen(res: list, btc_change: float, intervalo: int) -> str:
    ...
    if sl or ss:
        lines.append(f"🟢 STD ({len(sl)}L/{len(ss)}S):")
        for r,d in ([(r,'L') for r in sl[:2]]+[(r,'S') for r in ss[:2]]):
