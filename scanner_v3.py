...
# ─────────────────────────────────────────────────────────────────────
#  API BingX — FIX #3: recvWindow ANTES de firmar
# ─────────────────────────────────────────────────────────────────────
...
def _post(path: str, params: dict, retries: int = 3) -> Optional[dict]:
    for attempt in range(retries):
        ...
        try:
            ...
            log.error(f"POST {path} excepción ({attempt+1}/{retries}): {e}")
            if attempt < retries - 1:
                time.sleep(0.5 * (attempt + 1))
    return None
...
def build_alerta(par: dict) -> str:
    ...
    if AUTO_TRADE and (is_sup or is_fuel):
        ...
        lines.append(f"Verifica en TradingView 3m + QF x JP")
    return "\n".join(lines)
...
def build_resumen(res: list, btc_change: float, intervalo: int) -> str:
    ...
    lines=[
        ...
        f"W/L: {wr_str} | Racha: {consecutive_losses}{cb_str}",
        f"{'─'*24}",
    ]
