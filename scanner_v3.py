"""
CRYPTO SCANNER v3.3 — QF×JP ENGINE
FIX v3.3.1: BINGX_API_SECRET acepta también BINGX_SECRET
"""
...
class _HH(BaseHTTPRequestHandler):
    def do_GET(self):
        body = (f"OK scans={_hstatus['scan']} signals={_hstatus['signals']} "
                ...
                f"losses={_hstatus['losses']} last={_hstatus['last']}").encode()
...
def _run_http():
    try:
        ...
        print(f"[health] ERROR: {e}",flush=True); _http_ready.set()
...
# FIX: acepta BINGX_API_SECRET o BINGX_SECRET (cualquiera de los dos)
...
BLACKLIST_RAW   = os.getenv("BLACKLIST","ANIME-USDT,WCT-USDT,TAO-USDT,AAPLX-USDT,NCSKGOOGL2USD-USDT,VINE-USDT")
...
VOL_ATR_THR=0.60
MIN_VOLUME_USDT=5_000_000;TOP_N=10
    ...
    try:
        ...
        if isinstance(data,dict):
            ...
            if isinstance(bal,dict):
                for k in ["availableMargin","available","crossUnPnl"]:
                    if k in bal: return float(bal[k])
            for k in ["availableMargin","available"]:
                if k in data: return float(data[k])
        if isinstance(data,list):
            for asset in data:
                if asset.get("asset","").upper() in ("USDT",""):
                    for k in ["availableMargin","available"]:
                        if k in asset: return float(asset[k])
...
def linreg(arr,length):
    ...
    m,b=np.polyfit(x,y,1);return m*(length-1)+b
...
def analizar_par(klines_3m:list,klines_15m:list)->Optional[dict]:
    ...
    atr_now=float(atr[-1])
    atr_avg20_v=sma(atr,20)
    atr_avg20=float(atr_avg20_v[-1]) if not np.isnan(atr_avg20_v[-1]) else atr_now
    ...
    w_mom=I_W1+adx_f*I_W1*0.40;w_rev=max(I_W2*0.30,I_W2-adx_f*I_W2*0.50)
    ...
    vb=float(sma(v,I_DPB)[-1]);vs=bool(v[-1]>vb*I_DPM);rn=bool((h[-1]-l[-1])<atr_now*0.6)
    ...
    lconv=sum([norm_score>0.10,sig_alive,exec_ok,htf_bull,asym_bull,
               sell_exhausted,tl_break_long,dp_buy,cvd_rising,sq_bull or in_bull_fvg or in_bull_ob])
    sconv=sum([norm_score<-0.10,sig_alive,exec_ok,htf_bear,asym_bear,
               buy_exhausted,tl_break_short,dp_sell,not cvd_rising,sq_bear or in_bear_fvg or in_bear_ob])
    ...
    long_sup=long_fuel and comp_long>=SC_THR_SUP and (dp_buy or cvd_bull_div or sell_exhausted)
    short_sup=short_fuel and comp_short>=SC_THR_SUP and (dp_sell or cvd_bear_div or buy_exhausted)
    ...
    return {"signal":signal,"signal_score":ss,
...
def abrir_trade(symbol:str,precio:float,direccion:str)->Optional[dict]:
    ...
    if len(posiciones)>=MAX_OPEN_TRADES:
        log.warning(f"Máx trades ({MAX_OPEN_TRADES}) — skip {symbol}");return None
    balance=get_balance()
    log.info(f"Balance disponible: ${balance:.2f} USDT")
    if balance<TRADE_USDT:
        log.warning(f"Balance insuficiente (${balance:.2f}<${TRADE_USDT}) — skip {symbol}");return None
    ...
    if TRAILING_PCT>0:
        ...
        tp_desc=str(tp_p)
    trade={"symbol":symbol,"direction":direccion,"entry":precio,"sl":sl_p,"tp":tp_p,
           "tp_desc":tp_desc,"qty":qty,"usdt":TRADE_USDT,"leverage":LEVERAGE,
...
def abrir_trade_long(symbol,precio):return abrir_trade(symbol,precio,"LONG")
def abrir_trade_short(symbol,precio):return abrir_trade(symbol,precio,"SHORT")
    ...
    try:
        ...
        for sym in cerrados:
            ...
            if k:
                ...
                pnl=(pa-en)/en*100*(1 if il else -1)
                ganado=pnl>0
                if ganado:
                    _hstatus["wins"]+=1;consecutive_losses=0;res=f"✅ WIN +{pnl:.2f}%"
                else:
                    _hstatus["losses"]+=1;consecutive_losses+=1;res=f"❌ LOSS {pnl:.2f}%"
                    if consecutive_losses>=CB_MAX_LOSSES:
                        circuit_breaker_until=time.time()+CB_PAUSE_MIN*60
                        log.warning(f"⚡ Circuit breaker: {CB_MAX_LOSSES} pérdidas → pausa {CB_PAUSE_MIN}min")
                        send_telegram(f"⚡ *Circuit breaker*\n{consecutive_losses} pérdidas → pausa {CB_PAUSE_MIN}min")
                log.info(f"Trade cerrado: {sym} {trade['direction']} | {res}")
                send_telegram(f"📊 *Trade cerrado*: {sym.replace('-USDT','')}\n"
...
def build_alerta(par:dict)->str:
    ...
    sl_p=round(precio*(1-SL_PCT/100),6);tp1=round(precio*(1+TP_PCT/100),6)
    ...
    lines=[f"{emoji} *{sig}: {sym}*",f"{'─'*30}",
           ...
           f"{trailing_txt}{'─'*30}",f"*Dashboard QF x JP v3.3:*",
    ...
    if AUTO_TRADE and (is_sup or is_fuel):
        ...
        lines.append("Verifica en TradingView 3m + QF x JP")
    return "\n".join(lines)
...
def build_resumen(res:list,btc_change:float,intervalo:int)->str:
    ...
    lines=[f"QF x JP v3.3 — {now}",
           f"BTC {btce}{btc_change:.2f}% | prox {intervalo//60}min",
           f"W/L: {wr_str} | Racha: {consecutive_losses}{cb_str}",f"{'─'*24}"]
    ...
    for lst,lbl in [(sup_l,"LONG SUP"),(sup_s,"SHORT SUP"),(fl,"LONG FUEL"),(fs,"SHORT FUEL")]:
        if lst:
            ...
            for r in lst[:3]:
                ...
                lines.append(f"  {r['symbol'].replace('-USDT','')} {sc}/100")
    if sl2 or ss2:
...
# ── LOOP ──────────────────────────────────────────────────────────────
def run_loop():
    log.info(f"QF x JP Scanner v3.3 — AUTO_TRADE={'ON' if AUTO_TRADE else 'OFF'}")
    log.info(f"  ${TRADE_USDT}x{LEVERAGE} | SL {SL_PCT}% | TP {TP_PCT}% | Max {MAX_OPEN_TRADES} trades")
    log.info(f"  Trailing: {'ON '+str(TRAILING_PCT)+'%' if TRAILING_PCT>0 else 'OFF'}")
    log.info(f"  Blacklist: {BLACKLIST}")
    log.info(f"  Circuit breaker: {CB_MAX_LOSSES} perdidas -> pausa {CB_PAUSE_MIN}min")
    if BINGX_API_KEY:
        bal=get_balance();log.info(f"Balance inicial: ${bal:.2f} USDT")
    send_telegram(
        f"QF x JP Scanner v3.3 iniciado\n"
        f"AUTO TRADE: {'ON' if AUTO_TRADE else 'OFF'}\n"
        f"Config: ${TRADE_USDT} x {LEVERAGE} | SL {SL_PCT}% | TP {TP_PCT}%\n"
        f"Max trades: {MAX_OPEN_TRADES} | CB: {CB_MAX_LOSSES} perdidas -> {CB_PAUSE_MIN}min"
    )
    ultima_hora=-1;btc_change=0.0
    while True:
        try:
            ...
            for par in resultados:
                ...
                if AUTO_TRADE:
                    if (par["long_sup"] or par["long_fuel"]) and sym not in trades_abiertos:
                        ...
                        if trade:
                            send_telegram(f"LONG ABIERTO: {sym.replace('-USDT','')}\n"
                                          f"Entrada: {trade['entry']} SL: {trade['sl']} TP: {trade.get('tp_desc',trade['tp'])}\n"
                                          f"Qty: {trade['qty']} ${TRADE_USDT}x{LEVERAGE}")
                    elif (par["short_sup"] or par["short_fuel"]) and sym not in trades_abiertos:
                        trade=abrir_trade_short(sym,par["precio"])
                        if trade:
                            send_telegram(f"SHORT ABIERTO: {sym.replace('-USDT','')}\n"
                                          f"Entrada: {trade['entry']} SL: {trade['sl']} TP: {trade.get('tp_desc',trade['tp'])}\n"
                                          f"Qty: {trade['qty']} ${TRADE_USDT}x{LEVERAGE}")
