...
class PositionManager:
    ...
    async def _place_emergency_sl_all(self):
        """SL inmediato desde mark price actual para posiciones reconciliadas."""
        async with self._lock:
    ...
    async def _check_all_positions(self):
        ...
        # Sincronizar open_count + unrealized PnL con BingX real
        unrealized = sum(
            float(p.get("unrealizedProfit", p.get("unrealizedPnl", 0)) or 0)
            for p in real_positions if float(p.get("positionAmt", 0)) != 0
        )
        await self.risk.update_open_count(len(real_map), unrealized)
